				 ...
				 void *data_rx, size_t data_len)
{
	...
	if (ret < 0) {
		...
		LOG_HEXDUMP_WRN(tx_buf[0].buf, tx_buf[0].len, "tx_buf[0]");
		LOG_HEXDUMP_WRN(tx_buf[1].buf, tx_buf[1].len, "tx_buf[1]");

		if (req_rx || data_rx) {
			LOG_HEXDUMP_WRN(rx_buf[0].buf, rx_buf[0].len, "rx_buf[0]");
			LOG_HEXDUMP_WRN(rx_buf[1].buf, rx_buf[1].len, "rx_buf[1]");
		}
	}
...
void SX126xWriteCommand(RadioCommands_t opcode, uint8_t *buffer, uint16_t size)
{
	...
	LOG_ERR("Issuing opcode 0x%x w. %" PRIu16 " bytes of data", opcode, size);
	sx126x_spi_transceive(req, NULL, sizeof(req), buffer, NULL, size);
}
...
void SX126xReadBuffer(uint8_t offset, uint8_t *buffer, uint8_t size)
{
	...
	LOG_ERR("Reading buffers @ 0x%" PRIx8 " (%" PRIu8 " bytes)", offset, size);
	sx126x_spi_transceive(req, NULL, sizeof(req), NULL, buffer, size);
}
...
void SX126xWriteBuffer(uint8_t offset, uint8_t *buffer, uint8_t size)
{
	...
	LOG_ERR("Writing buffers @ 0x%" PRIx8 " (%" PRIu8 " bytes)", offset, size);
	sx126x_spi_transceive(req, NULL, sizeof(req), buffer, NULL, size);
}
